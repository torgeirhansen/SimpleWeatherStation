...
namespace SimpleWeatherStationBackgroundApp
{
    ...
    public sealed class HttpServer : IDisposable
    {
        ...
        public async void StartServerAsync(WeatherData w)
        {
            try
            {
        ...
        private async Task WriteResponseAsync(string[] requestTokens, IOutputStream outstream)
        {
            ...
            try
            {
                ...
                {
                    respBody = weatherData.JsonLastHour;
                }
                else if (urlPath.Equals("/LastDay", StringComparison.OrdinalIgnoreCase))
                {
                    respBody = weatherData.JsonLast24Hours;
                }
                else if (urlPath.Equals("/LastMonth", StringComparison.OrdinalIgnoreCase))
                {
                    respBody = weatherData.JsonLastMonth;
                }
                else
                {
                    respBody = weatherData.JsonCurrent;
                }
            }
            catch (Exception ex)
            {
